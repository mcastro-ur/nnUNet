...
class SplitBoundaryFromSeg:
    ...
    def __call__(self, **data_dict) -> dict:
        if self.meta_key not in data_dict:
            return data_dict  # rien à faire si on n'a pas fusionné avant

        n_bch = int(data_dict[self.meta_key])

        seg = data_dict[self.seg_key]  # (1 + C_small, Zp, Yp, Xp)
        ...
        data_dict[self.seg_key] = seg_main
        data_dict[self.boundary_key] = bnd_ch
